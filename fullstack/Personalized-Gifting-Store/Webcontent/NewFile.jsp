  ...
  <!-- Product Details -->
       ...
       <div class="row">
		  ...
		  <div id="imageContainer">
		    <img src="<%= product.getImageUrl() %>" id="image-1" alt="<%= product.getName() %>" onload="console.log('Image loaded successfully')">
		    <div id="image-2"></div>
		  </div>
		</div>
    ...
    <div class="col-md-6 product-details">
        ...
        <button class="btn btn-primary add-to-cart" data-product-id="<%= product.getId() %>">Add to Cart</button>
           <a href="easter.jsp" class="btn btn-link">Continue Shopping</a><br>
...
<p>
 <p style="font-size: 20px">Reviews : </p>
  ...
  console.log('Image 2:', image2);
    ...
    image2.style.backgroundImage = url('${image1.src}');
    
    imageContainer.addEventListener('mousemove', function(e) {
      const { left, top, width, height } = imageContainer.getBoundingClientRect();
      const x = e.clientX - left;
      const y = e.clientY - top;
      const zoomFactor = 1.2; // adjust zoom factor as needed
      
      image2.style.backgroundSize = ${image1.width * zoomFactor}px ${image1.height * zoomFactor}px;
      image2.style.backgroundPosition = -${x * zoomFactor - 87.5}px -${y * zoomFactor - 87.5}px;
      image2.style.left = ${x - 87.5}px; // 87.5px is half of image2's width
      image2.style.top = ${y - 87.5}px; // 87.5px is half of image2's height
    });
